...
void on_ant_azim_value_changed(GtkAdjustment *hscale, gpointer user_data);
void on_ant_elev_value_changed(GtkAdjustment *hscale, gpointer user_data);
void on_hnp_changed(GtkAdjustment *hscale, gpointer user_data);
void on_azim_trim_scale_value_changed(GtkAdjustment *hscale, gpointer user_data);
void on_elev_trim_scale_value_changed(GtkAdjustment *hscale, gpointer user_data);
void on_azim_pw_span_scale_value_changed(GtkAdjustment *hscale, gpointer user_data);
void on_elev_pw_span_scale_value_changed(GtkAdjustment *hscale, gpointer user_data);

void on_serial_port_combobox_changed(GtkComboBox *widget, gpointer user_data);
void on_reset_trims_button_clicked(GtkButton *button, gpointer user_data);
...
void on_save_settings_button_clicked(GtkButton *button, gpointer user_data);
...
char read_settings(const char* file_name);
char save_settings(const char* file_name );
...
char serial_open = 0;
  ...
  read_settings("ant_track_settings.txt");
  ...
  if (argc > 1) {
    ...
    for (x = 1; x < argc; x++) {
      if ((strncmp(argv[x], arg_string1, (sizeof(arg_string1) - 1))) == 0) {
        ...
        printf(" \n");
        printf("-------------------------------------------------------------------------------- \n");
        printf("Antenna Tracker V1.2 for the Paparazzi autopilot 13/February/2021 \n");
        printf("-------------------------------------------------------------------------------- \n");
        return 0;
      }
    }
...
unsigned char build_gui(void){
  ...
  builder = gtk_builder_new_from_file("ant_track_pmm.glade");
  ...
  azim_scale = GTK_WIDGET(gtk_builder_get_object(builder, "azim_scale"));
  ...
  //gtk_range_set_value(GTK_RANGE(azim_scale), (gdouble)(ant_azim+azimuth_trim));

  elev_scale = GTK_WIDGET(gtk_builder_get_object(builder, "elev_scale"));
  ...
  gtk_range_set_value(GTK_RANGE(elev_scale), (gdouble)(ant_elev+elevation_trim));
  ...
  elev_trim_scale = GTK_WIDGET(gtk_builder_get_object(builder, "elev_trim_scale"));
  ...
  gtk_range_set_value(GTK_RANGE(elev_trim_scale), (gdouble)elevation_trim);

  azim_trim_scale = GTK_WIDGET(gtk_builder_get_object(builder, "azim_trim_scale"));
  if (elev_trim_scale == NULL || !GTK_IS_WIDGET(elev_trim_scale)) {
    fprintf(stderr, "Unable to get Elevation Trim scale's address. (azim_trim_scale == NULL || azim_trim_scale != GtkWindow)\n");
    return 1;
  }
  gtk_range_set_value(GTK_RANGE(azim_trim_scale), (gdouble)azimuth_trim);

  elevation_pw_span_scale = GTK_WIDGET(gtk_builder_get_object(builder, "elev_pw_span_scale"));
  ...
  azimuth_pw_span_scale = GTK_WIDGET(gtk_builder_get_object(builder, "azim_pw_span_scale"));
  ...
  entry = GTK_WIDGET(gtk_builder_get_object(builder, "information1"));
  gtk_entry_set_text((GtkEntry *)entry, "Waiting for GPS lock!");

  azim_servo_pw_label = GTK_WIDGET(gtk_builder_get_object(builder, "azim_pw_label"));
  elev_servo_pw_label = GTK_WIDGET(gtk_builder_get_object(builder, "elev_pw_label"));
...
//%%%%%%%%%%%%%%%%%%%   GTK3 SIGNAL CALLBACK FUNCTIONS  %%%%%%%%%%%%%%%%%%%%%
...
void on_hnp_scale_value_changed(GtkAdjustment *hscale, gpointer user_data){
...
void on_serial_port_combobox_changed(GtkComboBox *widget, gpointer user_data)
{
  ...
  if (gtk_combo_box_get_active (combo_box) != 0) {
    gchar *serial_port_name = gtk_combo_box_text_get_active_text (GTK_COMBO_BOX_TEXT(combo_box));
    g_print ("You chose %s\n", serial_port_name);
    if (open_port(serial_port_name) == 0){
      gtk_entry_set_text (GTK_ENTRY(entry), "Port Opened Successfully");
    } else {
      gtk_entry_set_text (GTK_ENTRY(entry), "Failed to Open Serial Port");
    }
    //sleep(1);
    //gtk_entry_set_text (GTK_ENTRY(entry), "1 second after Failed to Open Serial Port");
...
void on_reset_settings_button_clicked(GtkButton *button, gpointer user_data){
   ...
   psi_servo_pw_span = 1000;
   theta_servo_pw_span = 1000;
   psi_servo_pw_span_default = 1000;
   theta_servo_pw_span_default = 1000;
   psi_servo_center_pw = 1500;
   theta_servo_center_pw = 1500;
   elevation_trim = 0;
   azimuth_trim = 0;

  //Update widgets with the new values.
  gtk_range_set_value(GTK_RANGE(elevation_pw_span_scale), (gdouble)psi_servo_pw_span_default);
  gtk_range_set_value(GTK_RANGE(azimuth_pw_span_scale), (gdouble)theta_servo_pw_span_default);
  gtk_range_set_value(GTK_RANGE(elev_trim_scale), (gdouble)elevation_trim);
  gtk_range_set_value(GTK_RANGE(azim_trim_scale), (gdouble)azimuth_trim);
...
void on_set_home_here_button_clicked(GtkButton *button, gpointer user_data){
...
void on_save_settings_button_clicked(GtkButton *button, gpointer user_data){
    ...
    save_settings("ant_track_settings.txt");
...
void on_main_window_destroy(GtkWidget *widget, gpointer user_data){
    ...
    //save_settings("ant_track_settings.txt");
    if (close(fd) == 0 ){
      printf("Success Serial Port Closed!");
    } else {
    ...
    if (tcflush(fd, TCIOFLUSH) == 0 ){
      ...
      printf("Fail to flush serial port!");
    }
    gtk_main_quit();
    //system("exit");
...
//%%%%%%%%%%%%%%%%%%%%%   SETTINGS READ/SAVE FILE FUNCTIONS  %%%%%%%%%%%%%%%%%%%%%%
...
char read_settings(const char* file_name){
  ...
  char* eof = NULL;
    ...
    if ( fp == NULL ){
      // create one file and reset values.
      printf( "ant_track_settings.txt file not found!\n" );
      printf( "A new one is created with Default values.\n" );
      psi_servo_pw_span = 1000;
      theta_servo_pw_span = 1000;
      psi_servo_pw_span_default = 1000;
      theta_servo_pw_span_default = 1000;
      ...
      elevation_trim = 0;
      azimuth_trim = 0;
      if (save_settings("ant_track_settings.txt") != 0){
        printf( "ant_track_settings.txt file failed to open.\n" );
        return(1);
      } else {
        fp = fopen(file_name, "r");
        if (fp == NULL){
      ...
      printf("The file is now opened for reading the settings.\n") ;
    }

    do{
        eof = fgets(file_line, sizeof(file_line), fp);
        if (eof != NULL) {
          sscanf(file_line, "%s %lf", var_name, &var_val);
          if (strstr(var_name, "ant_tracker_pan_mode")){
            ant_tracker_pan_mode = var_val;
            if (ant_tracker_pan_mode == 360) { hfov = 360; vfov = 90; } else { hfov = 180; vfov = 180; }
          }    
        }
    }while(eof != NULL);     
    ...
    do{
        ...
        if (eof != NULL) {
          sscanf(file_line, "%s %lf", var_name, &var_val);
          if (strstr(var_name, "psi_servo_pw_span_default")){
            psi_servo_pw_span_default = var_val;
          }    
        }
    }while(eof != NULL); 
    rewind(fp);
    ...
    do{
        ...
        if (eof != NULL) {
          sscanf(file_line, "%s %lf", var_name, &var_val);
          if (strstr(var_name, "theta_servo_pw_span_default")){
            theta_servo_pw_span_default = var_val;
          }    
        }
    }while(eof != NULL);     
    rewind(fp);
    ...
    do{
        ...
        if (eof != NULL) {
          sscanf(file_line, "%s %lf", var_name, &var_val);
          if (strstr(var_name, "psi_servo_center_pw")){
            psi_servo_center_pw = var_val;
          }    
        }
    }while(eof != NULL);     
    rewind(fp);
    ...
    do{
        ...
        if (eof != NULL) {
          sscanf(file_line, "%s %lf", var_name, &var_val);
          if (strstr(var_name, "theta_servo_center_pw")){
            theta_servo_center_pw = var_val;
          }    
        }
    }while(eof != NULL);     
    rewind(fp);
    ...
    do{
        ...
        if (eof != NULL) {
          sscanf(file_line, "%s %lf", var_name, &var_val);
          if (strstr(var_name, "elevation_trim")){
            elevation_trim = var_val;
          }    
        }
    }while(eof != NULL);     
    rewind(fp);
    ...
    do{
        ...
        if (eof != NULL) {
          sscanf(file_line, "%s %lf", var_name, &var_val);
          if (strstr(var_name, "azimuth_trim")){
            azimuth_trim = var_val;
          }    
        }
    }while(eof != NULL);     
    rewind(fp);
    printf("Settings were read successfully!\n") ;

/*
    // ANOTHER WAY OF READINFG THE FILE
    rewind(filePointer);
    fp = fopen("ant_track_settings.txt", "r") ;
    char len = fscanf(fp, "%s %lf %s %lf", str1, &psi_servo_pw_span_default, str2, &theta_servo_pw_span_default);
    if(char len == 0) { return(1); }
*/
...
// Declare the file pointer
  ...
  char str1[50];
    ...
    fp = fopen("ant_track_settings.txt", "w");
    if (fp == NULL){
      printf( "ant_track_settings.txt file failed to open." ) ;
      return(1);
    } else {
    ...
    sprintf(str1, "ant_tracker_pan_mode  %d \n", ant_tracker_pan_mode);
    fputs(str1, fp);   
    sprintf(str1, "psi_servo_pw_span_default  %d \n", (int)psi_servo_pw_span);
    fputs(str1, fp);   
    sprintf(str1, "theta_servo_pw_span_default  %d \n", (int)theta_servo_pw_span);
    fputs(str1, fp);   
    sprintf(str1, "psi_servo_center_pw  %d \n", (int)psi_servo_center_pw);
    fputs(str1, fp);   
    sprintf(str1, "theta_servo_center_pw  %d \n", (int)theta_servo_center_pw);
    fputs(str1, fp);   
    sprintf(str1, "elevation_trim  %lf \n", elevation_trim);
    fputs(str1, fp);   
    sprintf(str1, "azimuth_trim  %lf \n", azimuth_trim);
    fputs(str1, fp);   

    printf("Data successfully written in file ant_track_settings.txt\n");
    fclose(fp) ;
