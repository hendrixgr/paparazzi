 ...
 */
/*
#if defined(STM32F4) || defined(STM32F7) || defined(__DOXYGEN__)
...
#endif // #if defined(STM32F4) || defined(STM32F7) || defined(STM32H7) || defined(__DOXYGEN__)
*/

// Warning, flashdata_t must be unsigned!!!

//#define FLASH_CR_PSIZE_MASK       FLASH_CR_PSIZE_Msk
...
// Erase and the Write a sector using data from a 32bit array buffer
bool write_to_flash_32bit(uint8_t sector_nb, uint32_t* buffer_ptr, uint32_t size );

// Erase and the Write a sector using data from a 8bit array buffer.
bool write_to_flash_8bit(uint8_t sector_nb, const char* buffer_ptr, uint32_t size);


//Move the flash boot origin on STM32F7 and STM32H7
 ...
 * @return FLASH_RETURN_NO_PERMISSION   Access denied.
 */
int flash_Write(flashaddr_t address, const char* buffer, size_t size);
