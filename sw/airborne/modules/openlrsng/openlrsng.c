...
/**
 * @file modules/openlrsng/openlrsng.c
...
#define OUTPUTS 0 // outputs available

#define RC_LINK_STATUS_LED  2
...
enum{
       ...
       RFM_STATUS_BYTE_ARRIVED,
       RFM_STATUS_WAITING_RX_PACKET,
       RFM_STATUS_RX_PACKET_ARRIVED,
       RFM_STATUS_WAITING_TX_PACKET,
       RFM_STATUS_TX_PACKET_ARRIVED,
       RFM_STATUS_IDLE,
       RFM_STATUS_FINISHED
    };
...
static uint8_t spiReadRegister(uint8_t rfm_reg_address);
static void spi_read_registers_blocking(struct spi_transaction* rfm, char* buffer, uint8_t addr, uint8_t size);
static bool spiWriteRegister(uint8_t rfm_reg_address, uint8_t rfm_reg_val);

uint8_t handlePacketRX(void);
...
uint32_t timeout = 0;
  ...
  rfm_trans.output_buf[0] = rfm_reg_address & 0x7F;  // MSB 0 => read
...
uint8_t i = 0;
  ...
  rfm->output_buf[0] = addr & 0x7F;  // MSB = 0 means read operation, MSB = 1 means write operation
  rfm->output_length = 1;
  rfm->input_length = 0;
...
bool spiWriteRegister(uint8_t rfm_reg_address, uint8_t rfm_reg_val)
{
...
void check_module(void)
{
  if (rfmGetGPIO1() == 0) {
    ...
    LED_ON(RC_LINK_STATUS_LED);
    init_rfm(0);
    rx_reset();
    LED_OFF(RC_LINK_STATUS_LED);
  }
}
...
uint8_t i = 0;
  ...
  for (i = 0; i < size; i++) {
   buf[i] = rfm_trans.input_buf[i];  //spiReadData();
  }
...
void checkRSSI(void)
{
  ...
  if ((numberOfLostPackets < 2) && (lastRSSITimeUs != lastPacketTimeUs) && (pktTimeDelta > RSSI_timeout)) {
    ...
    if (RSSI_count > 8) {
      RSSI_sum /= RSSI_count;
      rx_RSSI = (((uint16_t)rx_RSSI * 3 + (uint16_t)RSSI_sum * 1) / 4);
      //set_RSSI_output();
      RSSI_sum = 0;
      RSSI_count = 0;
    }
...
void unpackChannels(uint8_t config, volatile uint16_t PPM_VALUES[], uint8_t *p)
{
  ...
  // Propagate the received 8 channel data to the airborne ppm handling array.
      ...
      switch((char) rxc_buf[0]) {
        ...
        case 'b': { // GET bind_data
          ...
          if (bind_data.version == BINDING_VERSION) {
            ...
            LED_ON(RC_LINK_STATUS_LED); //signal we got bound on LED:s
            return 1;
          }
        }
        break;
        // ALL THE BELOW CASES DO NOT APPLY HERE, THEY ARE HERE JUST FOR COMPATIBILITY.
...
uint8_t i = 0;
  ...
  LED_ON(RC_LINK_STATUS_LED);  //Green_LED_ON;
  ...
  // analy(s)ze the received packet's data
  ...
  // PREPARE THE DOWNLINK TELEMETRY PACKET
  linkAcquired = 1;
  disablePPM = 0;
...
void rfm_init(void) {
  ...
  //rxInitDefaults();
  rx_config.flags = ALWAYS_BIND;                         // FIXED VALUE, IT DOES NOT MATTER IN THIS APPLICATION
  ...
  rx_config.pwmStopDelay = 3;                            //300ms, IT DOES NOT MATTER IN THIS APPLICATION
...
uint8_t size = 0;
  ...
  switch(RF_Mode) {
    ...
    case(RECEIVED):
      LED_ON(RC_LINK_STATUS_LED);
      switch (rfm_status) {
        case(RFM_STATUS_IDLE):
          size = getPacketSize(&bind_data);
          rfm_trans.select = SPISelectUnselect;
          rfm_trans.output_buf[0] = RFM22B_FIFO;  // MSB 0 => read
          rfm_trans.output_length = 1;
        ...
        case (RFM_STATUS_RX_PACKET_ARRIVED):
          rfm_status = RFM_STATUS_IDLE;
          if (handlePacketRX() != NO_ERROR) {
            return;
          }
          break;
        default: break;
    ...
    case(TRANSMIT): return;
    
    case(TRANSMITTED):
      willhop = 1;
      rx_reset();          // Now we can set the RFM module for receiving again.
      timeUs = micros();
      timeMs = millis();
      pktTimeDelta = (timeUs - lastPacketTimeUs);  
      checkRSSI();
      if (linkAcquired) {
        checkLinkState();  // check RC link status after initial 'lock'
      } else if (pktTimeDelta > hopTimeoutSlow) {  // Still waiting for first packet, so hop slowly
        lastPacketTimeUs = timeUs;
        willhop = 1;
      }
...
uint8_t i = 0, size = 0;
  ...
  if (rfm_trans.status == SPITransSuccess) {
    ...
    switch (rfm_status) {
      ...
      case (RFM_STATUS_WAITING_RX_PACKET):
        size = getPacketSize(&bind_data);
        for (i = 0; i < size; i++) {
          rx_buf[idx] = rfm_trans.input_buf[idx+1];
        }
        rfm_status = RFM_STATUS_RX_PACKET_ARRIVED;
        break;

      default: rfm_status = RFM_STATUS_IDLE;
               break;
    }
  }
