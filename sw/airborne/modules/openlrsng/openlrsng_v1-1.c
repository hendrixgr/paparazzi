...
// 45 bytes, we use packing because if we allow padding the structure will have different size in 32bit cpus from that of AVRs
struct __attribute__((__packed__))  flash_data{
...
uint8_t getPacketSize(struct flash_data *ps);
...
uint32_t getInterval(struct flash_data *ps);
...
void init_rfm(uint8_t isbind)
{
  flashdata.rx_config.rx_type = RX_OLRSNG4CH;
  ...
  //rfmInit(flashdata.bind_data.flags&DIVERSITY_ENABLED);
  ...
  spiWriteRegister(RFM22B_FHS, flashdata.bind_data.rf_channel_spacing);
  uint32_t magic = isbind ? BIND_MAGIC : flashdata.bind_data.rf_magic;
  ...
  if (isbind) {
    ...
    rfmSetModemRegs(&modem_params[flashdata.bind_data.modem_datarate]);
    //rfmSetPower(bind_data.rf_power);
    spiWriteRegister(RFM22B_TXPOWER, flashdata.bind_data.rf_power);
    //delayMicroseconds(25); // PA ramp up/down time
    rfmSetCarrierFrequency(flashdata.bind_data.rf_frequency);
  }
...
void setHopChannel(uint8_t ch)
{
  uint8_t magicLSB = (flashdata.bind_data.rf_magic & 0xFF) ^ ch;
  rfmSetHeader(3, magicLSB);
  rfmSetChannel(flashdata.bind_data.hopchannel[ch]);
}
...
void updateHopChannel(void)
{
  if (willhop == 1) {
    RF_channel++;
    if ((RF_channel == MAXHOPS) || (flashdata.bind_data.hopchannel[RF_channel] == 0)) {
      RF_channel = 0;
    }
    //handleBeacon();
...
uint8_t getPacketSize(struct flash_data *ps)
{
  return pktsizes[(ps->bind_data.flags & 0x07)];
}
...
#define BYTES_AT_BAUD_TO_USEC(bytes, bps, div) ((uint32_t)((bytes) + (div?20:15)) * 8200000L / (uint32_t)(bps))
  ...
  ret = (BYTES_AT_BAUD_TO_USEC(getPacketSize(ps), modem_params[ps->bind_data.modem_datarate].bps, ps->bind_data.flags & DIVERSITY_ENABLED) + 2000);

  if (ps->bind_data.flags & TELEMETRY_MASK) {
    ret += (BYTES_AT_BAUD_TO_USEC(TELEMETRY_PACKETSIZE, modem_params[ps->bind_data.modem_datarate].bps, ps->bind_data.flags & DIVERSITY_ENABLED) + 1000);
  }
...
void checkLinkState(void)
{
  ...
  if (numberOfLostPackets) {
    ...
    if ( timeMsDelta > (flashdata.rx_config.ppmStopDelay * 100) ) {
      disablePPM = 1;
    ...
    if (RF_Mode == RECEIVED) {
      ...
      switch((char) rxc_buf[0]) {
        ...
        case 'b': { // GET bind_data
          memcpy(&flashdata.bind_data, (rxc_buf + 1), sizeof(flashdata.bind_data));
          if (flashdata.bind_data.version == BINDING_VERSION) {
...
void handlePacketRX(void)
{
  ...
  if (RF_Mode == RECEIVED) {
      ...
      rfmGetPacket(rx_buf, getPacketSize(&flashdata));
      ...
      if ((rx_buf[0] & 0x3e) == 0x00) {  // this packet has the servo values.
        unpackChannels(flashdata.bind_data.flags & 7, PPM, rx_buf + 1);
...
void rfm_init(void) {
  ...
  //bindInitDefaults();
  flashdata.bind_data.version = BINDING_VERSION;
  flashdata.bind_data.serial_baudrate = DEFAULT_BAUDRATE;
  flashdata.bind_data.rf_power = DEFAULT_RF_POWER;
  flashdata.bind_data.rf_frequency = DEFAULT_CARRIER_FREQUENCY;
  flashdata.bind_data.rf_channel_spacing = DEFAULT_CHANNEL_SPACING;
  flashdata.bind_data.rf_magic = DEFAULT_RF_MAGIC;
  memset(flashdata.bind_data.hopchannel, 0, sizeof(default_hop_list));
  memcpy(flashdata.bind_data.hopchannel, default_hop_list, sizeof(default_hop_list));

  flashdata.bind_data.modem_datarate = DEFAULT_DATARATE;
  flashdata.bind_data.flags = DEFAULT_FLAGS;

  //if ((flashdata.rx_config.flags & ALWAYS_BIND) && (!(flashdata.rx_config.flags & SLAVE_MODE))) {
    if (bindReceive(500)) {
      //bindWriteEeprom();
      write_to_flash_8bit(11, (char*)&flash_data.bind_data, sizeof(flash_data.bind_data));
    }
  //}

  // NOT ALL SETTINGS ARE USED!
  //rxInitDefaults();
  flashdata.rx_config.flags = ALWAYS_BIND;
  flashdata.rx_config.RSSIpwm = 255;                 // off, IT DOES NOT MATTER IN THIS APPLICATION
  flashdata.rx_config.failsafeDelay = 10;            //1s, IT DOES NOT MATTER IN THIS APPLICATION
  flashdata.rx_config.ppmStopDelay = 3;              //300ms, IT DOES NOT MATTER IN THIS APPLICATION
  flashdata.rx_config.pwmStopDelay = 3;              //300ms
  flashdata.rx_config.beacon_frequency = DEFAULT_BEACON_FREQUENCY; // IT DOES NOT MATTER IN THIS APPLICATION
  flashdata.rx_config.beacon_deadtime = DEFAULT_BEACON_DEADTIME;   // IT DOES NOT MATTER IN THIS APPLICATION
  flashdata.rx_config.beacon_interval = DEFAULT_BEACON_INTERVAL;   // IT DOES NOT MATTER IN THIS APPLICATION
  flashdata.rx_config.minsync = 3000;

  flashdata.rx_config.rx_type = RX_MICRO;
  flashdata.rx_config.pinMapping[0] = PINMAP_PPM;     // IT DOES NOT MATTER IN THIS APPLICATION
  flashdata.rx_config.pinMapping[1] = PINMAP_ANALOG;  // IT DOES NOT MATTER IN THIS APPLICATION
  flashdata.rx_config.pinMapping[2] = PINMAP_RSSI;    // IT DOES NOT MATTER IN THIS APPLICATION
  flashdata.rx_config.pinMapping[3] = PINMAP_ANALOG;  // IT DOES NOT MATTER IN THIS APPLICATION
  flashdata.rx_config.pinMapping[4] = PINMAP_RXD;     // IT DOES NOT MATTER IN THIS APPLICATION
  flashdata.rx_config.pinMapping[5] = PINMAP_TXD;     // IT DOES NOT MATTER IN THIS APPLICATION
/*
  flashdata.rx_config.rx_type = RX_OLRSNG4CH;  
  flashdata.rx_config.pinMapping[0] = PINMAP_PPM;     // IT DOES NOT MATTER IN THIS APPLICATION
  flashdata.rx_config.pinMapping[1] = PINMAP_ANALOG;  // IT DOES NOT MATTER IN THIS APPLICATION
  flashdata.rx_config.pinMapping[2] = PINMAP_RSSI;    // IT DOES NOT MATTER IN THIS APPLICATION
  flashdata.rx_config.pinMapping[3] = PINMAP_ANALOG;  // IT DOES NOT MATTER IN THIS APPLICATION
  flashdata.rx_config.pinMapping[4] = 4;              // IT DOES NOT MATTER IN THIS APPLICATION
  flashdata.rx_config.pinMapping[5] = 5;              // IT DOES NOT MATTER IN THIS APPLICATION
  flashdata.rx_config.pinMapping[6] = PINMAP_RXD;     // IT DOES NOT MATTER IN THIS APPLICATION
  flashdata.rx_config.pinMapping[7] = PINMAP_TXD;     // IT DOES NOT MATTER IN THIS APPLICATION
*/
  // READ SETTINGS FROM STM32F405 FLASH AND LOAD THEM TO THE RAM.  
  flash_Read(flashSectorBegin(11), (char*)&flash_data.bind_data, sizeof(flash_data.bind_data) );  
...
*/
  ...
  hopcount=0;
  while ((hopcount < MAXHOPS) && (flashdata.bind_data.hopchannel[hopcount] != 0)) {
    hopcount++;
  }
  ...
  serial_tail = 0;
  linkAcquired = 0;
  hopInterval = getInterval(&flashdata);
